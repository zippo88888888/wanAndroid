<template>
  <div id="address_list">
    ...
    <div class="al-list m-b" v-for="(item,index) in addressList" :key="index" v-on:click="itemClick(item)">
      <div class="al-item-1">
        <span class="al-item1-name m-t">{{item.name}}</span>
        <span class="al-item1-mobile m-t">{{item.phone}}</span>
      </div>
      <div class="al-item-address m-t txt-2">{{item.address}}{{item.addressInfo}}</div>
      <div class="line"></div>
...
export default {
  ...
  methods: {
    itemClick: function (item) {
      ...
      if (this.type == 1) {
        console.log('从确认订单跳转的，需要将值传到确认订单')
